...
#   WAYWALLEN_APPIMAGE_WEBENGINE  ON for embedded QtWebEngine Workshop browser, OFF for Lite, default OFF
...
case "${WAYWALLEN_APPIMAGE_WEBENGINE,,}" in
    ...
    0|off|false|no|lite|minimal)
        ...
        ;;
    *)
        fail "invalid WAYWALLEN_APPIMAGE_WEBENGINE=$WAYWALLEN_APPIMAGE_WEBENGINE (use ON or OFF)"
        ;;
esac
...
if [[ -x "$HERE/usr/libexec/QtWebEngineProcess" ]]; then
    ...
    mkdir -p         "$WAYWALLEN_STEAM_WEBENGINE_HOME/profile"         "$WAYWALLEN_STEAM_WEBENGINE_HOME/cache"         "$WAYWALLEN_STEAM_WEBENGINE_HOME/chromium-user-data"         "$WAYWALLEN_STEAM_WEBENGINE_HOME/chromium-cache"
