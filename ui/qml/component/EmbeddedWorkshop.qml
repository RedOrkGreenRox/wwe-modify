...
Item {
    ...
    // Keep Steam cookies outside the AppImage mount and outside temporary
    // AppImage runtime directories. StandardPaths returns a file:// URL in QML;
    // QtWebEngine expects plain filesystem paths for persistent profile storage.
    ...
    WebEngineView {
        ...
        Keys.onPressed: event => {
            ...
            if (event.key === Qt.Key_F5 || (event.key === Qt.Key_R && hasCtrl)) {
                web.reload();
                event.accepted = true;
            } else if ((event.key === Qt.Key_F && hasCtrl) || (hasCtrl && txt === 'а')) {
